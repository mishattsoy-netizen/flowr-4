    ...
    >
      ...
      <Tooltip content="Drag / Options">
        <div
          draggable
          onDragStart={(e) => {
            const blockElement = (e.currentTarget as HTMLElement).closest('.editor-block');
            if (blockElement) {
          ...
          className={clsx(
            ...
            (menuOpen || isDragging) && "bg-white/10 text-foreground"
          )}
        >
...
export function BlockRenderer({
  ...
  const isOver = dragState?.overId === block.id;
  ...
  if (block.type === 'divider') {
    return (
      ...
      >
        <DropIndicator active={isOver && dragState?.overPosition === 'above'} position="above" />
        <BlockControls {...controlsProps} />
        <div className={clsx(
          "flex items-center w-full py-4 relative group rounded-xl transition-colors", 
          isSelected && "bg-white/[0.01]",
          isDragging && "opacity-0"
        )}>
          <div className="flex-1 h-px bg-border/50" />
        </div>
        <DropIndicator active={isOver && dragState?.overPosition === 'below'} position="below" />
      </div>
    );
  }
  ...
  if (block.type === 'image' || block.type === 'video') {
    ...
    if (!isImage) {
      if (videoUrl.includes('youtube.com') || videoUrl.includes('youtu.be')) {
        ...
        videoUrl = `https://player.vimeo.com/video/${vid}`;
      }
    }
    ...
    return (
      <div className={clsx(
        ...
        <div className={clsx(
          ...
          isSelected && "bg-white/[0.01] rounded-xl",
          isDragging && "opacity-0 invisible"
        )}>
          <div className={clsx("relative group/media border border-white/5 bg-white/5", widthClass, "rounded-xl ")}>
  ...
  if (block.type === 'embed') {
    ...
    return (
      ...
      >
        <BlockControls {...controlsProps} />
        <div className={clsx(
          "relative w-full transition-colors rounded-xl", 
          isSelected && "bg-white/[0.01]",
          isDragging && "opacity-0"
        )}>
          <div onClick={() => linked && setActiveEntityId(linked.id)} className="border border-white/5 rounded-xl px-5 py-4 hover:bg-white/5 flex items-center gap-4 transition-colors">
  ...
  if (block.type === 'column') {
    return (
      ...
      >
        <DropIndicator active={isOver && dragState?.overPosition === 'above'} position="above" />
        <BlockControls variant="column" blockId={block.id} menuOpen={menuOpen} onInsertAfter={onInsertAfter} onOpenMenu={onOpenMenu} onDragStart={onDragStart} onDragEnd={onDragEnd} />
        <div className={clsx("flex flex-col gap-2 relative z-10", isDragging && "opacity-0 invisible")}>
          {(block.children || []).map((subBlock, sIdx) => (
            <BlockRenderer key={subBlock.id} block={subBlock} index={sIdx} onUpdate={onUpdate} onDelete={onDelete} onInsertAfter={onInsertAfter} onSlash={onSlash} onOpenMenu={onOpenMenu} onFocus={onFocus} isInsideColumn={true} dragState={dragState} onDragStart={onDragStart} onDragOver={onDragOver} onDragEnd={onDragEnd} />
          ))}
        </div>
        <DropIndicator active={isOver && dragState?.overPosition === 'below'} position="below" />
      </div>
  ...
  if (block.type === 'columns') {
    return (
      ...
      >
        <DropIndicator active={isOver && dragState?.overPosition === 'above'} position="above" />
        <div className={clsx("flex gap-4 w-full h-full relative z-10 group", isDragging && "opacity-0 invisible")}>
          <BlockControls {...controlsProps} />
          <div className="flex gap-4 w-full h-full">
            {(block.children || []).map((colBlock, cIdx) => (
              <div key={colBlock.id} className="relative flex-1 basis-0 min-w-0 flex flex-col">
                <BlockRenderer block={colBlock} index={cIdx} onUpdate={onUpdate} onDelete={onDelete} onInsertAfter={onInsertAfter} onSlash={onSlash} onOpenMenu={onOpenMenu} onFocus={onFocus} isInsideColumn={true} dragState={dragState} onDragStart={onDragStart} onDragOver={onDragOver} onDragEnd={onDragEnd} />
              </div>
            ))}
          </div>
        </div>
