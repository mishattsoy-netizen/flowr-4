    ...
    return {
      p: ({ node, children }: any) => {
        ...
        const isEmpty = !children || (Array.isArray(children) && children.length === 0) || (typeof children === 'string' && !children.trim());
          ...
          {(atEnd && !isEmpty) && <span className="ai-cursor-inline">Γûê</span>}
        </div>;
      },
      a: ({ href, children }: any) => {
      ...
      strong: ({ node, children }: any) => {
        ...
        return <strong className="font-bold">{children}{atEnd && <span className="ai-cursor-inline">Γûê</span>}</strong>;
      },
      ...
      li: ({ node, children }: any) => {
        ...
        return (
          <li className="flex items-start gap-2.5">
            <span className="text-accent select-none shrink-0 mt-[0.45em] flex items-center justify-center leading-none" aria-hidden="true">ΓÇó</span>
              <div className="flex-1 min-w-0 leading-relaxed font-medium text-[13.5px] tracking-wide break-words !max-w-full !w-full">
              {children}
              {atEnd && <span className="ai-cursor-inline">Γûê</span>}
            </div>
          </li>
        );
      },
      code: ({ node, children }: any) => {
        const atEnd = !hasFinishedTyping && isAtEnd(node);
        return <code className="bg-white/10 rounded px-1.5 py-0.5 text-[12px] font-mono tracking-wide font-medium">{children}{atEnd && <span className="ai-cursor-inline">Γûê</span>}</code>;
      },
