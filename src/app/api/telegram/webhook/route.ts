  ...
  try {
    ...
    try {
      ...
      if (result.type === 'photo') {
        await telegram.sendPhoto(chatId, result.content as Buffer, `Generated by Flowr AI ✨`)
        logInteraction(chatId, '[IMAGE GENERATED]', 'model', 'image', result.usage_type || 'chat', result.status || 'success', result.model_chain).catch(e => logger.error('Model image log failed', e))
        ...
        logInteraction(chatId, result.content as string, 'model', 'text', result.usage_type || 'chat', result.status || 'success', result.model_chain).catch(e => logger.error('Model log failed', e))
