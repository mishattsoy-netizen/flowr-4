  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="widget instrument-hover cursor-default">
          <div className="text-[10px] font-black text-bone-60 tracking-[0.1em] uppercase mb-3 opacity-40">Total registered</div>
          <div className="text-3xl font-black text-bone-100 tracking-tighter leading-none font-instrument">{users?.length || 0}</div>
        </div>
        <div className="widget instrument-hover cursor-default">
          <div className="flex items-center justify-between mb-3 opacity-40">
            <span className="text-[10px] font-black text-bone-60 tracking-[0.1em] uppercase">Active today</span>
            <Users className="w-4 h-4 text-accent" strokeWidth={1.5} />
          </div>
          <div className="text-3xl font-black text-accent tracking-tighter leading-none font-instrument">{activeCount}</div>
        </div>
        <div className="widget instrument-hover cursor-default">
          <div className="flex items-center justify-between mb-3 opacity-40">
            <span className="text-[10px] font-black text-bone-60 tracking-[0.1em] uppercase">Blocked nodes</span>
            <ShieldAlert className="w-4 h-4 text-rose-500" strokeWidth={1.5} />
          </div>
          <div className="text-3xl font-black text-rose-500 tracking-tighter leading-none font-instrument">{blockedCount}</div>
        </div>
      </div>
